...
class ODBCDataExtractor(DataExtractor):
    def __init__(self, config):
        ...
        connstr = config.args.dbfile and odbc.connstr % config.args.dbfile or odbc.connstr
        ...
        if odbc.table:
            ...
            tables = [tb.table_name for tb in self.cursor.tables() if tb.table_type == 'TABLE']
    ...
    def getrows(self, top=0):
        ...
        if top > 0:
            topstr = 'top %s' % top
        sql = '''select %s * from [%s]''' % (topstr, self.table,)
...
class ODBCDataLoader(DataLoader):
    ...
    @staticmethod
    def _get_insert_sql(cols, table):
